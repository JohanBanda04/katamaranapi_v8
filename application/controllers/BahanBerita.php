...
class BahanBerita extends CI_Controller
{
    var $table_name = 'bahan_berita';
    ...
    public function bahanBeritaByTanggal($tanggal) //parameter didapet dari url
    {
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'GET' || $this->uri->segment(3) == '') {
            ...
            if ($check_auth_client == true) {
                $response = $this->MyModel->auth();
                if ($response['status'] == 200) {
                    $resp = $this->MyModel->bahanBeritaByTanggal($tanggal);
                    json_output($response['status'], $resp);
                }
            }
        }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if (empty($params['berita_what'])) {//ISI NAMA PARAMETER INPUT POST NYA
                        $respStatus = 400;
